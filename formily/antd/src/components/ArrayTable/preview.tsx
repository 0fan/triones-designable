...
import {genArrayTableStyle} from "./styles";
...
export const ArrayTable: DnFC<TableProps<any>> = observer((props) => {
    ...
    const prefix = "dn-array-table"
    const {hashId, wrapSSR} = useCssInJs({prefix, styleFun: genArrayTableStyle})
    ...
    return wrapSSR(
        <div {...nodeId} className={cls(prefix, hashId)}>
            {renderTable()}
            <LoadTemplate
                actions={[
                    {
