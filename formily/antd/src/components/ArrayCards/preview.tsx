...
import {TreeNode, createResource} from '@0fan/designable-core'
import {
    useTreeNode,
    TreeNodeWidget,
    DroppableWidget,
...
import {genArrayCardsStyle} from "./styles";
...
export const ArrayCards: DnFC<CardProps> = observer((props) => {
    ...
    const {hashId, wrapSSR} = useCssInJs({prefix: "dn-array-cards", styleFun: genArrayCardsStyle})
    const renderCard = () => {
    ...
    return wrapSSR(
        <div {...nodeId} className={cls("dn-array-cards", hashId)}>
            {renderCard()}
            <LoadTemplate
                actions={[
                    {
